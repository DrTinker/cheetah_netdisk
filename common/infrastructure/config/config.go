...
func (c *ConfigClientImpl) GetDiscoveryConfig() (*models.DiscoveryConfig, error) {
	//判断配置是否加载成功
	if c.source == nil {
		return nil, errors.New("empty mq config")
	}

	c.DisCovery.Address = c.source.Section("DisCovery").Key("address").MustString("127.0.0.1")
	c.DisCovery.Tool = c.source.Section("DisCovery").Key("tool").MustString("consul")
	c.DisCovery.Port = c.source.Section("DisCovery").Key("port").MustInt(8500)
